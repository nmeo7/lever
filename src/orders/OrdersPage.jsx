...
const RecordPaymentModal = ({ order, onClose }) => {
	...
	return (
		<Modal
			title={`Record payment — order ${order.id.slice(0, 8)}`}
			open
					...
					<table className='w-full text-sm'>
						<thead>
							<tr style={{ background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)' }}>
								{['Date', 'Items', 'Total', 'Status', 'Payment', ''].map((col) => (
						...
						<tbody>
							{orders.map((order, i) => (
								<tr
									...
									<td className='px-4 py-3' style={{ color: 'var(--color-text)' }}>{order.items?.length ?? 0} item(s)</td>
