...
import { fetchSuppliers, createSupplier, batchUpsertSuppliers } from './suppliersApi'

const COLUMNS = ['companyName', 'contactName', 'email', 'phone', 'address']

const SupplierCard = ({ supplier }) => (
	<div className='neu-raised rounded-2xl p-4 flex flex-col gap-1.5' style={{ background: 'var(--neu-bg)' }}>
		<p className='text-sm font-semibold truncate' style={{ color: 'var(--color-text)' }}>{supplier.companyName}</p>
		{supplier.contactName && <p className='text-xs' style={{ color: 'var(--color-muted)' }}>{supplier.contactName}</p>}
		{supplier.email && <p className='text-xs' style={{ color: 'var(--color-muted)' }}>{supplier.email}</p>}
		{supplier.phone && <p className='text-xs' style={{ color: 'var(--color-muted)' }}>{supplier.phone}</p>}
		{supplier.address && <p className='text-xs' style={{ color: 'var(--color-muted)' }}>{supplier.address}</p>}
		{supplier.notes && <p className='text-xs line-clamp-2' style={{ color: 'var(--color-muted)' }}>{supplier.notes}</p>}
	</div>
)
...
const BulkImportPanel = () => {
	...
	const { mutate: commit, isPending, error } = useMutation({
		mutationFn: batchUpsertSuppliers,
		onSuccess: (data) => {
			setResult(data)
			queryClient.invalidateQueries({ queryKey: ['suppliers'] })
		},
		...
		<div className='rounded-2xl p-5 border mb-6' style={{ borderColor: 'var(--color-border)' }}>
			<div className='flex items-center gap-2.5 mb-4'>
				...
				<div>
					...
					<p className='text-xs' style={{ color: 'var(--color-muted)' }}>Create suppliers from a CSV</p>
				</div>
			</div>
			...
			{!rows.length ? (
				...
				<div className='flex flex-col gap-3'>
					...
					<button
						...
						{isPending ? 'Importing…' : `Import ${rows.length} suppliers`}
					</button>
				</div>
			)}
			...
			{result && (
				<div className='flex flex-col gap-1.5 mt-3'>
					<p className='flex items-center gap-1.5 text-xs' style={{ color: 'var(--color-text)' }}>
						...
						Imported {result.count} suppliers
					</p>
