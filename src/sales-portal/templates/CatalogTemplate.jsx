...
const usePublishedProducts = (companyId) =>
	useQuery({
		...
		queryFn: async () => {
			const snap = await getDocs(query(collection(db, 'erp-products'), where('companyId', '==', companyId), where('published', '==', true)))
			return snap.docs.map((d) => ({ id: d.id, ...d.data() }))
		},
		enabled: !!companyId,
	})

const formatCurrency = (amount) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'FRW' }).format(amount ?? 0)
...
const HeroSearch = ({ products, whatsappUrl }) => {
	...
	const results = trimmed
		? products.filter(
				(p) => p.name?.toLowerCase().includes(trimmed) || p.description?.toLowerCase().includes(trimmed) || p.category?.toLowerCase().includes(trimmed),
			)
		: []
	...
	return (
		<div className="relative w-full max-w-xl">
			<form onSubmit={handleSubmit}>
				<div className="flex items-center gap-3 rounded-2xl px-5 py-4" style={{ background: 'var(--color-primary)' }}>
					<svg
						width="16"
						height="16"
						viewBox="0 0 24 24"
			...
			{showDropdown && (
				<div
					...
					style={{ background: 'var(--color-surface)', border: '2px solid var(--color-border)' }}>
					{results.length === 0 ? (
						<div className="px-5 py-4 text-sm flex items-center justify-between gap-4" style={{ color: 'var(--color-muted)' }}>
						...
						results.slice(0, 6).map((p) => (
							<button
								...
								<span className="text-xs font-semibold flex-shrink-0" style={{ color: 'var(--color-primary)' }}>
...
const MiniSearch = ({ products }) => {
	...
	const trimmed = input.trim().toLowerCase()
	const results = trimmed ? products.filter((p) => p.name?.toLowerCase().includes(trimmed) || p.category?.toLowerCase().includes(trimmed)) : []
		...
		<div className="relative flex-1 max-w-xs">
			...
			{trimmed && (
				<div
					...
					style={{ background: 'var(--color-surface)', border: '2px solid var(--color-border)' }}>
					{results.length === 0 ? (
						<p className="px-4 py-2 text-lg" style={{ color: 'var(--color-muted)' }}>
...
const buildSections = (categories, products, featuredLabel) => {
	const featuredSection = { label: featuredLabel, image: categories[0]?.image, products }

	const categorySections = categories.map((cat) => ({ ...cat, products: products.filter((p) => p.category?.toLowerCase() === cat.label.toLowerCase()) }))

	return [featuredSection, ...categorySections].filter((s) => s.products.length > 0)
}
...
const CategoryPanel = ({ category, isActive, onClick, panelRef }) => (
	<button
		...
		{category.image && (
			<img src={category.image} alt={category.label} className="absolute inset-1 w-[calc(100%-8px)] h-[calc(100%-8px)] object-cover rounded-xl" />
		)}
		{category.image && <div className="absolute inset-1 rounded-xl" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.55), rgba(0,0,0,0.1))' }} />}
		<p className="absolute bottom-0 left-0 right-0 text-xs font-semibold p-2" style={{ color: category.image || isActive ? '#fff' : 'var(--color-text)' }}>
			{category.label}
		</p>
	</button>
)
...
const WhatsappQrPanel = ({ whatsappUrl, whatsapp, className = '' }) => {
	...
	return (
		<a
			...
			<p className="text-xs font-semibold uppercase tracking-widest" style={{ color: '#ffffff', opacity: 0.85 }}>
				{t('salesPortal.chatWithUs', 'Chat with us')}
			</p>
			<div className="rounded-xl p-3" style={{ background: '#ffffff' }}>
				<QRCode value={whatsappUrl} size={110} color="#1f2a24" bgColor="transparent" bordered={false} />
			</div>
...
const CatalogTemplate = ({ org, whatsapp, whatsappUrl }) => {
	...
	const scrollToSection = (index) => sectionRefs.current[index]?.scrollIntoView({ behavior: 'smooth' })

	const categoryPanelRefs = useRef([])
	categoryPanelRefs.current = sections.map((_, i) => categoryPanelRefs.current[i] ?? null)

	useEffect(() => {
		categoryPanelRefs.current[activeIndex]?.scrollIntoView({ behavior: 'smooth', inline: 'nearest', block: 'nearest' })
	}, [activeIndex])
	...
	return (
		<div className="flex flex-col gap-24">
			{/* Hero */}
			<section className="text-center flex flex-col items-center gap-4 pt-4 pb-4">
				...
				<p className="text-lg max-w-xl mb-8" style={{ color: 'var(--color-muted)' }}>
					{org?.frontdesk?.subheadline ?? 'Hano tubafitiye amata na fanta bikonje!'}
				</p>
				{showSearch && (
			...
			{showCatalog && (
				<section id="products" className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-8 items-start max-w-6xl mx-auto w-full">
					<div className="flex flex-col gap-12 min-w-0">
						<div className="sticky top-16 z-40 -mx-4 px-4 pt-0 flex flex-col gap-3" style={{ background: 'var(--color-background)' }}>
							{showSearch && (
								<div
									className="flex items-center gap-4 overflow-hidden transition-all duration-200"
									style={{ height: showMiniHeader ? '4rem' : '0px', opacity: showMiniHeader ? 1 : 0 }}>
									<p className="text-sm font-bold flex-shrink-0" style={{ color: 'var(--color-text)' }}>
						...
						{sections.map((section, index) => (
							<CategorySection key={section.label} section={section} sectionRef={(el) => (sectionRefs.current[index] = el)} />
						))}
					</div>

					<div className="sticky top-20 flex flex-col gap-4">
						{showWhatsapp && <WhatsappQrPanel whatsappUrl={whatsappUrl} whatsapp={whatsapp} />}
			...
			{!showCatalog && showWhatsapp && (
				<section className="flex justify-center">
					<WhatsappQrPanel whatsappUrl={whatsappUrl} whatsapp={whatsapp} className="w-full max-w-xs" />
				</section>
			)}
