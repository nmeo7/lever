...
import { fetchLocations, createLocation, batchUpsertLocations, LOCATION_TYPES, LOCATION_STATUSES } from './locationsApi'

const COLUMNS = ['name', 'type', 'address', 'description', 'status']
...
const LocationCard = ({ location }) => (
	<div className='neu-raised rounded-2xl p-4 flex flex-col gap-1.5' style={{ background: 'var(--neu-bg)' }}>
		...
		{location.address && <p className='text-xs' style={{ color: 'var(--color-muted)' }}>{location.address}</p>}
		{location.description && <p className='text-xs line-clamp-2' style={{ color: 'var(--color-muted)' }}>{location.description}</p>}
		<span
			className='text-[10px] font-semibold uppercase tracking-wide w-fit px-2 py-0.5 rounded-full mt-1'
			style={{ background: 'var(--color-background)', color: location.status === 'active' ? 'var(--color-primary)' : 'var(--color-muted)' }}>
			{location.status}
		</span>
	</div>
)

const AddLocationModal = ({ open, onClose }) => {
	...
	return (
		<Modal
			...
			<Form form={form} layout='vertical' onFinish={mutate} className='pt-2'>
				...
				<Form.Item name='type' label='Type' initialValue='warehouse' rules={[{ required: true }]}>
				...
				<Form.Item name='status' label='Status' initialValue='active' rules={[{ required: true }]}>
					<Select options={LOCATION_STATUSES} />
				</Form.Item>
...
const BulkImportPanel = () => {
	...
	const { mutate: commit, isPending, error } = useMutation({
		mutationFn: batchUpsertLocations,
		onSuccess: (data) => {
			setResult(data)
			queryClient.invalidateQueries({ queryKey: ['locations'] })
		},
		...
		<div className='rounded-2xl p-5 border mb-6' style={{ borderColor: 'var(--color-border)' }}>
			<div className='flex items-center gap-2.5 mb-4'>
				...
				<div>
					...
					<p className='text-xs' style={{ color: 'var(--color-muted)' }}>Create locations from a CSV</p>
				</div>
			</div>
			...
			{!rows.length ? (
				...
				<div className='flex flex-col gap-3'>
					...
					<button
						...
						{isPending ? 'Importing…' : `Import ${rows.length} locations`}
					</button>
				</div>
			)}
			...
			{result && (
				<div className='flex flex-col gap-1.5 mt-3'>
					<p className='flex items-center gap-1.5 text-xs' style={{ color: 'var(--color-text)' }}>
						...
						Imported {result.count} locations
					</p>
