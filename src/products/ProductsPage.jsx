	...
	<div
		...
		<div className='h-32 flex items-center justify-center' style={{ background: 'var(--color-background)' }}>
			{product.imageUrl ? (
				...
				<span className='text-xs' style={{ color: 'var(--color-muted)' }}>No image</span>
			)}
		</div>
	...
	return (
		<Modal
			...
			<Form form={form} layout='vertical' onFinish={mutate} className='pt-2'>
				...
				<Form.Item name='productType' label='Type' initialValue='physical' rules={[{ required: true }]}>
					<Select options={PRODUCT_TYPES.map(({ value, label }) => ({ value, label }))} />
				</Form.Item>

				<Form.Item name='sellingPrice' label='Price'>
				...
				<div className='flex items-center gap-3'>
					<div className='relative max-w-md flex-1'>
						...
						<input
							...
							placeholder='Search products by meaning, e.g. "monthly plans"'
